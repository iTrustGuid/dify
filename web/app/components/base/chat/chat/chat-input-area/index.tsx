...
const ChatInputArea = ({
  ...
  const featureButtons: FeatureButton[] = [
    { id: "handle", label: "智能办件", isExpandable: true },
    { id: "query", label: "智能查询", isExpandable: true },
    { id: "consult", label: "业务咨询", isExpandable: true },
    { id: "more", label: "更多", isExpandable: true },
  ];
  ...
  const aliyunConfigRef = useRef<{ token: string; appKey: string } | null>(
    null,
  );
        ...
        return (
          <div className="p-4">
            <div className="space-y-3">
              <button
                onClick={() => sendQuickMessage("办理一手房转移业务")}
                className="block w-full text-left text-sm text-gray-900"
              >
                一手房转移
              </button>
              <button
                onClick={() => sendQuickMessage("办理补证换证业务")}
                className="block w-full text-left text-sm text-gray-900"
              >
                补证换证
              </button>
              <button
                onClick={() => sendQuickMessage("开具有房无房证明")}
                className="block w-full text-left text-sm text-gray-900"
              >
                开具证明
              </button>
              <button
                onClick={() => sendQuickMessage("退出业务办理模式")}
                className="block w-full text-left text-sm text-gray-900"
              >
                退出业务
              </button>
      ...
      case "query":
        return (
          <div className="p-4">
            <div className="space-y-3">
              <button
                onClick={() => sendQuickMessage("查询我的证照")}
                className="block w-full text-left text-sm text-gray-900"
              >
                我的证照
              </button>
              <button
                onClick={() => sendQuickMessage("查询我的房产")}
                className="block w-full text-left text-sm text-gray-900"
              >
                我的房产
              </button>
              <button
                onClick={() => sendQuickMessage("查询我的业务进度")}
                className="block w-full text-left text-sm text-gray-900"
              >
                业务进度
              </button>
      ...
      case "consult":
        return (
          <div className="p-4">
            <div className="space-y-3">
              <button
                onClick={() => sendQuickMessage("新房办证业务怎么办理")}
                className="block w-full text-left text-sm text-gray-900"
              >
                新房办证
              </button>
              <button
                onClick={() => sendQuickMessage("买卖二手房业务怎么办理")}
                className="block w-full text-left text-sm text-gray-900"
              >
                房屋买卖
              </button>
              <button
                onClick={() => sendQuickMessage("购买新房怎么核算税费")}
                className="block w-full text-left text-sm text-gray-900"
              >
                新房缴税
              </button>
  ...
  // 确保识别结果实时同步到输入框
  const updateRecognitionResult = useCallback(
    (text: string) => {
      if (isStoppingRef.current) return;
  ...
  const startRecognition = async () => {
    ...
    try {
      ...
      const ws = new WebSocket(
        `${ALIYUN_URL}?appkey=${configData.appKey}&token=${configData.token}`,
      );
  ...
  return (
    <>
      ...
      <FeatureButtonBar
        ...
        onExpandClick={(id) => setActivePanel(id)}
      />
          ...
          <div ref={wrapperRef} className="flex items-center justify-between">
            <div className="relative flex w-full grow items-center">
              ...
              <Textarea
                ...
                placeholder={decode(
                  t("common.chat.inputPlaceholder", { botName }) ||
                    "请输入消息",
                )}
                autoFocus={false}
                minRows={1}
